...
For this reason, to save 32 bytes of transaction data when creating immutable pools, We checking the instruction data length and parse the data accordingly; if the data is shorter, we set aut`hority to `None`; if it includes the full `authority` field, we parse it and wrap it in `Some()`.
