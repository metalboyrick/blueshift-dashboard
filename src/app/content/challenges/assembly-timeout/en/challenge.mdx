...
> This is not "safe" alone. We don't check if the `MAX_SLOT_HEIGHT` will be at the right offset since we don't check that we only have on account. Additionally we don't check that the address of the `Clock` account is correct. But this is an add-on instruction meaning that it should be ran in good fate.
