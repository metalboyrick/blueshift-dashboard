...
Now that you should starting to be comfortable writing Assembly, instead of using the `sol_get_clock_sysvar` try achieving the same goal by passing in the `Clock` account.
...
<RequirementList>
  <Requirement
    title="Challenge 1: Testing the Timeout"
    description="Your program should suceed when a slot higher than the current one is passed."
  />

  <Requirement
    title="Challenge 2: Testing the Failure of a Timeout"
    description="Your program should fail when a slot lower than the current one is passed."
  />
</RequirementList>