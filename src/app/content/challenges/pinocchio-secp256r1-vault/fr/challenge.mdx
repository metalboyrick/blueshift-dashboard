...
Ajoutez Pinocchio et le crate `Secp256r1` compatible avec Pinocchio
...
Bien que *unwrap* soit généralement déconseillé en production, dans ce cas précis il est utilisé en toute sécurité car nous avons déjà validé la longueur des données dans la méthode `try_from`. Si la longueur des données n'est pas correcte, une erreur sera renvoyée avant d'atteindre ce point.
...
`Secp256r1Pubkey` a une longueur de 33 octets car elle utilise une représentation par points compressés pour les clés publiques à courbe elliptique. Ce format comprend :
