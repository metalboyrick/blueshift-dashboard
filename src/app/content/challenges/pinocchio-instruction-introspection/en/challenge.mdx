...
The entrypoint, that lives in the `lib.rs` looks very similar to what we talked about in the [Introduction to Pinocchio Course](/en/courses/introduction-to-pinocchio)
...
Before diving into the `loan` and `repay` instruction, let's have a look at the `helpers.rs`:
...
As you can see, there isn't much. There is a `LoanData` struct that we're going to use as throwaway account data to save the data of the loans before the `repay` instruction and the `get_token_amount()` helper that gives us the amount present in the token account.