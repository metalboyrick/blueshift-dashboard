...
> The field must be available in the struct, like this:
...
- `SizeDiscriminatorNode`: Used to describe discrimination based on size. It takes a `number` as the size parameter:
...
```ts
const constantPda = pdaNode({
    name: 'tickets',
    seeds: [constantPdaSeedNodeFromString('utf8', 'tickets')],
}); 

const variablePda = pdaNode({
    name: 'ticket',
...
In the next section we'll examine all these elements in detail.
