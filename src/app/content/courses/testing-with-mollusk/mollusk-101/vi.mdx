...
Bao gồm phàn bổ trợ cụ thể cho chương trình khi cần:
...
Một số crate Solana nâng cao trải nghiệm kiểm thử bằng cách cung cấp các type và utility thiết yếu:

```
cargo add solana-precompiles solana-account solana-pubkey solana-feature-set solana-program solana-sdk
```

Vì những phần phụ thuộc này chỉ cần thiết cho kiểm thử, hãy giữ cho binary chương trình của bạn nhẹ bằng cách thêm chúng dưới phần `[dev-dependencies]` trong `Cargo.toml` của bạn:

```
[dev-dependencies]
mollusk-svm = "0.4.0"
```
> Cấu hình này đảm bảo rằng các utility kiểm thử không làm tăng kích thước triển khai chương trình của bạn trong khi cung cấp quyền truy cập vào tất cả các type Solana cần thiết và hàm bổ trợ trong quá trình phát triển.
...
- Uninitialized Account: Một program account đang chờ khởi tạo trong instruction của bạn
...
Hoặc sử dụng `AccountSharedData`:
...
> Khi sử dụng AccountSharedData, hãy nhớ gọi `.into()` để chuyển đổi nó thành type Account mà Mollusk yêu cầu để thực thi như thế này: `program_account_account.into()`.
