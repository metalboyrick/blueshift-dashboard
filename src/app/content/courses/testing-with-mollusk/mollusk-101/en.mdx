...
> We can perform multiple checks on the same account by "bundling" them together like this: `Check::account(&payer).data(&expected_data).owner(&ID).lamports(expected_lamports).build()`
