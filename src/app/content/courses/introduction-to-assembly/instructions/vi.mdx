...
<ArticleSection name="Phân loại và định dạng lệnh" id="instruction-categories-and-format" level="h2" />

Khi bạn biên dịch mã Rust, C, hoặc assembly, chuỗi công cụ sẽ tạo ra một luồng các lệnh có độ rộng cố định 8-byte được đóng gói vào phần `.text` của ELF.
...
- `opcode`: Xác định loại thao tác. 3 bit cao nhất chọn lớp lệnh (số học, bộ nhớ, nhảy, gọi, thoát), trong khi 5 bit thấp hơn chỉ định biến thể chính xác (cộng, nhân, tải, nhảy-nếu-bằng).
- `dst`: Số thanh ghi đích (`r0–r10`) nơi kết quả được lưu trữ—kết quả số học, giá trị đã tải, hoặc giá trị trả về từ hàm trợ giúp.
...
### Các danh mục chỉ dẫn

Các loại chỉ dẫn khác nhau sử dụng các trường này theo những cách cụ thể:
...
Mã hóa mã thao tác nắm bắt nhiều thông tin ngoài loại thao tác:
- Lớp chỉ dẫn: Số học, bộ nhớ, nhảy, gọi, v.v.
- Kích thước thao tác: Thao tác 32-bit so với 64-bit
- Loại nguồn: Thanh ghi so với giá trị tức thời
- Thao tác cụ thể: Cộng so với trừ, tải so với lưu trữ, v.v.

Điều này tạo ra các mã thao tác riêng biệt cho các biến thể chỉ dẫn. Ví dụ, `add64 r1, r2` (nguồn thanh ghi) sử dụng mã thao tác khác với `add64 r1, 42 ` (nguồn tức thời). Tương tự, `add64` và `add32` có các mã thao tác khác nhau cho các kích thước thao tác khác nhau.

Các thao tác số học phân biệt thêm giữa các biến thể có dấu và không dấu. `udiv64` xử lý giá trị như không dấu (0 đến 18 quintillion), trong khi `sdiv64` xử lý các giá trị có dấu (-9 quintillion đến +9 quintillion).

<ArticleSection name="Thực thi chỉ dẫn" id="instruction-execution" level="h2" />

Mã thao tác xác định cách VM diễn giải các trường còn lại.

Khi VM gặp `add64 r1, r2`, nó đọc mã thao tác và nhận ra đây là một thao tác số học 64-bit sử dụng hai thanh ghi:

Trường `dst` chỉ ra kết quả đi vào `r1`, trường `src` chỉ định `r2` là toán hạng thứ hai, và các trường `offset` và `immediate` bị bỏ qua.

Đối với `add64 r1, 42`, mã thao tác thay đổi để chỉ ra một thao tác tức thời. Bây giờ `dst` vẫn trỏ đến `r1`, nhưng `src` trở nên vô nghĩa, và trường `immediate` cung cấp toán hạng thứ hai (42).

Các thao tác bộ nhớ kết hợp nhiều trường một cách có ý nghĩa:

Đối với `ldxdw r1, [r2+8]`, mã thao tác chỉ ra một thao tác tải bộ nhớ 64-bit, `dst` nhận giá trị được tải, `src` cung cấp địa chỉ cơ sở, và `offset` (8) được thêm vào để tạo địa chỉ cuối cùng `r2 + 8`.
...
Cơ chế gọi của sBPF đã phát triển qua nhiều phiên bản để có độ rõ ràng và bảo mật tốt hơn. Cho đến sBPF v3, `call imm` phục vụ hai mục đích: giá trị tức thời xác định liệu bạn đang gọi một hàm nội bộ hay đang gọi một syscall. 

Thời gian chạy phân biệt giữa các loại này dựa trên phạm vi giá trị tức thời, với các số syscall thường là các số nguyên dương nhỏ như 16 cho `sol_log`.

Từ sBPF v3 trở đi, các lệnh được tách biệt để có hành vi rõ ràng. `call` giờ đây xử lý các lệnh gọi hàm nội bộ bằng cách sử dụng offset tương đối, trong khi `syscall imm` gọi rõ ràng các hàm thời gian chạy. Sự tách biệt này làm rõ ý định của bytecode và cho phép xác minh tốt hơn.
...
| opcode | Mnemonic                 | Mô tả                         |
|--------|--------------------------|--------------------------------|
| stw    | `stw [dst + off], imm`   | Lưu trữ từ tức thời            |
| sth    | `sth [dst + off], imm`   | Lưu trữ nửa từ tức thời        |
| stb    | `stb [dst + off], imm`   | Lưu trữ byte tức thời          |
| stdw   | `stdw [dst + off], imm`  | Lưu trữ từ kép tức thời        |
| stxw   | `stxw [dst + off], src`  | Lưu trữ từ từ thanh ghi        |
| stxh   | `stxh [dst + off], src`  | Lưu trữ nửa từ từ thanh ghi    |
| stxb   | `stxb [dst + off], src`  | Lưu trữ byte từ thanh ghi      |
| stxdw  | `stxdw [dst + off], src` | Lưu trữ từ kép từ thanh ghi    |
