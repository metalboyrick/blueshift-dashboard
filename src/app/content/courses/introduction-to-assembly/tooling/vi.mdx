...
Chúng ta bắt đầu bằng cách cài đặt gói bằng cách:
...
- `deploy`: Xây dựng và triển khai chương trình.
- `test`: Kiểm tra chương trình đã triển khai.
- `e2e`: Xây dựng, triển khai và kiểm tra chương trình.
- `clean`: Dọn dẹp các tệp xây dựng và triển khai.
...
Sau khi khởi tạo dự án, bạn có thể điều hướng vào thư mục dự án và sử dụng các lệnh khác để xây dựng, triển khai và kiểm tra chương trình của bạn.

<ArticleSection name="Xây dựng chương trình" id="building-programs" level="h2" />

sBPF cung cấp hai tùy chọn xây dựng tùy thuộc vào thiết lập và nhu cầu của bạn:

### Xây dựng sBPF

Lệnh xây dựng sử dụng một trình hợp ngữ độc lập không yêu cầu thêm các phụ thuộc. Nó hoàn hảo để học tập và thử nghiệm vì nó "hoạt động ngay lập tức" trên bất kỳ nền tảng nào.
