...
Pour les programmes très complexes, vous pouvez organiser la logique en modules distincts :
...
1. **Garder des Instructions Ciblées** : Chaque instruction doit permettre de faire une chose correctement. Si une instruction est trop complexe, envisagez de la diviser en plusieurs instructions.
...
**Remarque** : Vous pouvez trouver tous les CPIs du Programme du Système en utilisant la crate Anchor principale : `use anchor_lang::system_program::*`. Pour celui relatif au Programme de Jetons SPL, nous devrons importer le crate anchor_spl et faire : `use anchor_spl::token::*`
