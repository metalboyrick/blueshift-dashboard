...
Pour conserver la structure existante, le discriminateur ne se trouve pas au premier octet comme d'habitude mais à l'octet `165`.

En effet, la longueur du compte de `Token` est de 164 octets ce qui signifie que le discriminateur est ajouté après la longueur de base. Pour le compte de `Mint` cela signifie que nous avons dû ajouter 83 octets de remplissage (*padding*) afin de garantir que les deux comptes aient la même longueur de base.

Pour distinguer les deux comptes, il suffit donc de vérifier le 165e octet et d'agir en conséquence.
