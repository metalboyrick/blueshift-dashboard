...
`ImmutableOwner` (Propriétaire immuable) est une extension de compte de `Token` qui empêche toute modification de la propriété du compte de `Token`. Cela protège les comptes contre les accès non autorisés et les tentatives de transfert.
