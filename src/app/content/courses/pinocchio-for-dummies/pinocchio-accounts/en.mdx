...
Now things get more interesting. We start with the usual account check (ownership and lenght check), but we also add other specific functions to provide additional helpers that resemble Anchor macros like `init` and `init_if_needed`.
