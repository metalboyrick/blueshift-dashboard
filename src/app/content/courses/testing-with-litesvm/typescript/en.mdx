...
You will also need to serialize the account data into a buffer, which can be done either manually or using a library such as `@coral-xyz/borsh` (see example [here](https://solana.com/ru/developers/courses/native-onchain-development/serialize-instruction-data-frontend)).
