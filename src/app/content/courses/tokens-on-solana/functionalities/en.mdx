...
> This functions just initialize the account, so we need to make sure that we use the `createAccount()` instruction from the System Program before and setting the correct owner (token program), space and lamports to make it rent exampt.
