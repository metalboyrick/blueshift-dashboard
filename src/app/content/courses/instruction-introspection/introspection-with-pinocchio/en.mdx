...
Pinocchio has different types than the one used by the `solana_program` crate this means that we had to create a new SDK that was optimized for pinocchio.
...
For this reason we can use "Instruction Introspection" in pinocchio without having to install any external crate and access all the information that we need by importing this functions:
...
We usually start by checking if the program used is the one that we expect. In this example we're introspecting another instruction of the system program, so we can do it like so:
